...
class MantencionesController extends AbstractActionController
{
    ...
    //
        $titulo = "Plan de Mantenciones";
        //$color  = "#D9EDF7";
      //  $resultado = $prueba->getDatos('1');                                                           
        return new ViewModel(array('titulo'=>$titulo,'color'=>$color));
    }
   //////////////////////////////////////////////////////////////////////// NOTAS                                         
    ...
    {   
        ...
        $result = new ViewModel(array('dif'=>$dif,'cierre'=>$descr_cierre));            
    ...
    public function getnotasAction()
    {
        ...
        //Retornamos a la vista
        $result = new ViewModel(array('notas'=>$notas));            
        $result->setTerminal(true);                
        return $result;                
    }
    ...
    {            
        ...
        //Validamos si es insert o cargamos formulario.
            ...
            }else if (isset($lista['id_partida'])){
                //Obtenemos partida de mantencion
                $partida = $part->getPartidaId($lista['id_partida']);
                
                $form->get('nombre')->setAttribute('value' ,$partida[0]['accion']." / ".$partida[0]['nombre']);
    ...
    {   
        ...
        $partidas = $part->getPartidasMes(date('M'));
        $partidas2 = $part->getPartidasMes(date('M',strtotime('+1 month',strtotime(date('M')))));
        $partidas3 = $part->getPartidasMes(date('M',strtotime('+2 month',strtotime(date('M'))))); 
